...
function inputChange(i, name, price,image) {
    console.log(`I want to buy the ${i} item named ${name} that cost $${price} dollars `);
    let listItem = document.querySelectorAll('li')[i];
    let listInput = listItem.querySelector('input');
    let button = listItem.querySelector('button');

    button.onclick = function(){
        cart.push({
            quantity: listInput.value,
            name: name,
            price:price,
            image:image
        })
